...
const roles = [
  { icon: Share2, title: "Social Media Manager", slug: "social-media-manager", agent: "Content Agent", description: "Plans content, drafts captions, organizes posting ideas, and helps keep your brand active online." },
  { icon: Headphones, title: "Customer Support", slug: "customer-support", agent: "Support Agent", description: "Handles common customer questions, organizes replies, and helps maintain fast, professional communication." },
  { icon: Mail, title: "Email Marketer", slug: "email-marketer", agent: "Campaign Agent", description: "Builds email campaigns, follow-up sequences, promotions, and retention messaging." },
  { icon: CalendarCheck, title: "Virtual Assistant", slug: "virtual-assistant", agent: "Admin Agent", description: "Helps with scheduling, reminders, admin tasks, follow-ups, and business organization." },
];
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {roles.map((role, i) => (
          <motion.div key={role.title} initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}>
            <Link to={`/ai-employees/${role.slug}`} className="card-glass group block cursor-pointer rounded-xl p-6">
