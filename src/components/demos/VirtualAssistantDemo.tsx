...
const VirtualAssistantDemo = () => {
  const { state, isConnected, addConnection, removeConnection, updateTaskStatus, setAssistantExpectations } = useAppState();
    ...
    <>
      ...
      {demoStep === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">Tell your AI assistant about your work so it can anticipate needs and stay organized.</p>
          {[
      ...
      {demoStep === 2 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">Define how your AI assistant should operate day-to-day.</p>
          {[
            { key: "taskTypes", label: "Task Types", placeholder: "Admin, Scheduling, Follow-ups, Research" },
            ...
            { key: "approvalNeeded", label: "What Needs Your Approval", placeholder: "Client-facing outputs, Schedule changes" },
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => { setExpectationsSaved(true); setTimeout(() => setDemoStep(3), 500); }} className="btn-glow text-sm">
      ...
      {demoStep === 3 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">Manage task execution. Approve, hold, or complete items.</p>
          <div className="space-y-3">
              ...
              <div key={task.id} className={`rounded-xl border p-4 transition-all ${
                ...
                {task.status !== "completed" && (
                  <div className="flex gap-2">
                    <button onClick={() => updateTaskStatus(task.id, "completed")} className="flex items-center gap-1 rounded-lg bg-emerald-500/10 px-3 py-1.5 text-xs font-medium text-emerald-400 hover:bg-emerald-500/20"><Check size={12} /> Complete</button>
      ...
      {demoStep === 4 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { label: "Tasks Today", value: String(state.tasks.filter(t => t.status !== "completed").length), icon: FileText },
              { label: "Completed", value: String(state.tasks.filter(t => t.status === "completed").length), icon: Check },
              { label: "Reminders", value: "4", icon: Bell },
              { label: "Meetings", value: "3", icon: Calendar },
            ].map((stat) => (
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-xl border border-border/50 bg-card p-5">
              ...
              <div className="space-y-2 text-sm text-muted-foreground">
                <p>• {state.tasks.filter(t => t.status === "completed").length} tasks completed today</p>
                <p>• 2 follow-ups pending from yesterday</p>
                <p>• Client call at 2 PM — agenda prepared</p>
                <p>• Weekly report due by 5 PM</p>
              </div>
            </div>
            <div className="rounded-xl border border-border/50 bg-card p-5">
              <h4 className="mb-3 font-display text-sm font-semibold text-foreground">Upcoming</h4>
              <div className="space-y-2 text-sm text-muted-foreground">
                <p>• Team sync — Tomorrow 10 AM</p>
                <p>• Vendor follow-up — Tomorrow 2 PM</p>
                <p>• Monthly review — Friday 11 AM</p>
              </div>
            </div>
          </div>
